defmodule HousingApp.ManagementTest do
  ...
  describe "profiles" do
    ...
    test "list_profiles/0 returns all profiles" do
      ...
      user_tenant2 = user_tenant_fixture(%{"tenant_id" => tenant2.id, "user_id" => user2.id, "role" => :admin}, actor: user2)
    ...
    test "Regular user can't read another user's profile" do
      tenant = tenant_fixture()
      user1 = user_fixture()
      user_tenant1 = user_tenant_fixture(%{"tenant_id" => tenant.id, "user_id" => user1.id, "role" => :admin}, actor: user1)
      {:ok, user_profile1} = Management.create_profile(%{"user_tenant_id" => user_tenant1.id}, tenant.id, actor: user_tenant1)

      user2 = user_fixture(%{"email" => "foo2@bar.com"})
      user_tenant2 = user_tenant_fixture(%{"tenant_id" => tenant.id, "user_id" => user2.id, "role" => :user}, actor: user2)
