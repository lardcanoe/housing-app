defmodule HousingAppWeb.EnsureRolePlugTest do
  use HousingAppWeb.ConnCase

  alias HousingAppWeb.EnsureRolePlug
  ...
  setup do
    conn =
      ...
      |> Pow.Plug.put_config(otp_app: :housing_app)
      |> fetch_flash()
  ...
  test "call/2 with non-admin user", %{conn: conn} do
    ...
    conn =
      conn
      |> Pow.Plug.assign_current_user(@user, otp_app: :housing_app)
  ...
  test "call/2 with admin user", %{conn: conn} do
    ...
    conn =
      conn
      |> Pow.Plug.assign_current_user(@admin, otp_app: :housing_app)
      |> EnsureRolePlug.call(opts)
